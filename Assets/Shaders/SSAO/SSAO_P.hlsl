...
float4 PSMain(VS_Output input) : SV_TARGET
{
    ...
    float3 fragPos = worldTex.Sample(samplerLinear, input.texCoord).xyz;
    float3 normal = normalTex.Sample(samplerLinear, input.texCoord).xyz;
    float3 randomVec = noiseTex.Sample(samplerLinear, input.texCoord * noiseScale).xyz;
    ...
    for(uint i = 0; i < kernelSize; ++i)
    {
        ...
        float sampleDepth = worldTex.Sample(samplerLinear, offset.xz).z;
